...
:::info
AgentFlow must be installed before using these configurations. If you haven't installed it yet, please see the [Installation Guide](installation).
:::

## Getting API Keys

To use AgentFlow with the recommended configuration, you'll need to obtain the following API key:
...
AgentFlow will automatically detect the available keys and select the default provider. If only `GEMINI_API_KEY` is set, it will use the `gemini` provider and the `gemini-2.5-pro-preview-03-25` model.

## Basic Usage

Start AgentFlow in interactive chat mode:
...
```bash
agentflow -m "Help me understand this code"
```

If you prefer to use aider's specialized code editing capabilities instead of AgentFlow's built-in file modification tools:

```bash
agentflow -m "Implement this feature" --use-aider
```
...
agentflow -m "Your task" --log-mode file --log-level debug

# Log everything to console
agentflow -m "Your task" --log-mode console --log-level info
```

For more detailed logging configuration, see the [Logging documentation](../configuration/logging.md).

For information on AgentFlow's memory management and how to reset memory when needed, see the [Memory Management documentation](../configuration/memory-management.md).
