...
AgentFlow's planning workflow allows you to generate detailed implementation plans without executing any code. This powerful feature enables you to review, approve, and understand the AI's proposed approach before committing time and resources to execution.
...
agentflow -m "Create a simple snake game using Pygame" --research-and-plan-only

# Using the short form
agentflow -m "Add user authentication to my Flask app" -rap
```
...
agentflow extract-last-plan > implementation-plan.md

# Or view it with your preferred pager
agentflow extract-last-plan | less
```
...
```bash
# 1. Generate the plan
agentflow -m "Refactor my_script.py to be more modular" --research-and-plan-only

# 2. Extract and review the plan
agentflow extract-last-plan > refactor-plan.md
# (Review the plan in your editor)

# 3. If satisfied, execute the same task without the flag
agentflow -m "Refactor my_script.py to be more modular"
```
...
agentflow -m "Build a weather CLI tool" --research-and-plan-only
...
agentflow -m "Build a weather CLI tool with current weather and 3-day forecast" --research-and-plan-only

# 4. Extract final plan
agentflow extract-last-plan > weather-cli-plan.md
```

### Example 3: Manual Implementation Guide

Use AgentFlow to create detailed guides for manual implementation:

```bash
# 1. Generate comprehensive plan
agentflow -m "Set up a new React project with TypeScript, ESLint, and testing" --research-and-plan-only

# 2. Extract as implementation guide
agentflow extract-last-plan > react-setup-guide.md
...
agentflow -m "Migrate our authentication system from JWT to OAuth2" --research-and-plan-only

# Extract and share with team
agentflow extract-last-plan > migration-proposal.md
...
agentflow extract-last-research-notes

# Save research notes to file
agentflow extract-last-research-notes > research-findings.md
```
...
agentflow -m "Build API" --research-and-plan-only --project-state-dir ~/my-project-state

# Extract plan from custom directory
agentflow extract-last-plan --project-state-dir ~/my-project-state
```
...
Plans are stored in AgentFlow's SQLite database as markdown text:

- **Location**: `.agentflow/pk.db` (or custom location with `--project-state-dir`)
...
Each AgentFlow run creates a new session with a unique ID:

- Session IDs are displayed when AgentFlow starts
...
The planning workflow integrates with AgentFlow's memory system:
...
agentflow -m "Add feature X" --research-and-plan-only --project-state-dir /team/shared/project-state

# Team members can extract the same plan
agentflow extract-last-plan --project-state-dir /team/shared/project-state
```
...
```bash
# Plan phase
agentflow -m "Add error handling to database module" --research-and-plan-only

# Review phase
agentflow extract-last-plan

# Execution phase (if plan looks good)
agentflow -m "Add error handling to database module"
```
...
**Problem**: `agentflow extract-last-plan` returns "No plan found for the latest session."
...
- Check the session logs in `.agentflow/logs/` for error details
...
**Solutions**:
- Session IDs are printed when AgentFlow starts: `"Initialized SessionRepository"`
- Use `agentflow extract-plan` without an ID to get the latest session
...
**Solutions**:
- Ensure the `.agentflow` directory exists and is writable
...
**Problem**: `agentflow extract-last-research-notes` finds no notes.
...
agentflow -m "Migrate user table to add email verification fields" --research-and-plan-only

# Extract and review
agentflow extract-last-plan > db-migration-plan.md

# Extract research for additional context
agentflow extract-last-research-notes > migration-research.md
```
...
agentflow -m "Add real-time chat functionality to the web app" --research-and-plan-only

# Save comprehensive documentation
agentflow extract-last-plan > chat-feature-plan.md
agentflow extract-last-research-notes > chat-research.md
...
agentflow -m "Optimize database queries" --research-and-plan-only --custom-tools ./tools/db_tools.py

# Extract plan that includes custom tool recommendations
agentflow extract-last-plan > optimization-plan.md
```