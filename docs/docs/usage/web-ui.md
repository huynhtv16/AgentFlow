...
AgentFlow includes a modern web-based user interface that allows you to interact with the agent through your browser. This interface offers a convenient alternative to the command-line experience, with real-time streaming of agent output and an intuitive chat-like interaction.

## Launching the Web UI

To start AgentFlow with the web interface, use the `--server` flag:
...
agentflow --server --server-host 127.0.0.1 --server-port 3000
```
...
3. Watch as AgentFlow processes your request in real-time
...
- For SSL/TLS support, you'll need to configure a reverse proxy like Nginx in front of the AgentFlow server