...
## Introduction
AgentFlow's memory system enables the agent to persist knowledge across sessions, creating a more efficient and coherent experience. The memory stores key facts, code snippets, and research notes about your project, allowing AgentFlow to build on previous work and avoid redundant operations.

This persistent memory gives AgentFlow the ability to:
...
AgentFlow's memory is stored in a SQLite database file located in a hidden `.agentflow` directory within your current working directory. The specific file path is:
...
AgentFlow implements the repository pattern for database access, with specialized repositories for each memory type:
- `KeyFactRepository`
...
To prevent uncontrolled growth of the memory database, AgentFlow implements automatic garbage collection that triggers when specific thresholds are exceeded:
- Facts: > 50 items
- Snippets: > 35 items
- Notes: > 30 items
...
AgentFlow provides a CLI flag to completely reset the memory database:

```bash
agentflow --wipe-project-memory [other arguments]
```

This flag deletes the entire `.agentflow/pk.db` database file, giving you a fresh start with no stored memory.

If you're using a custom project state directory, combine both flags:

```bash
agentflow --wipe-project-memory --project-state-dir /path/to/custom/directory [other arguments]
```

This will delete the database file at `/path/to/custom/directory/pk.db` instead of the default location.

The memory statistics are displayed in the status panel when you start AgentFlow, showing:
...
**Solution**: 
1. Ensure AgentFlow has properly shut down before starting a new session
2. If issues persist, use `--wipe-project-memory` to recreate the database
3. Check the logs in `.agentflow/logs/` for specific errors

### Issue: Memory items seem irrelevant
**Solution**:
- Let the automatic garbage collection work by continuing to use AgentFlow
- For immediate reset, use `--wipe-project-memory`

### Issue: Missing .agentflow directory
**Solution**: The directory is automatically created when you run AgentFlow. If it's missing, simply run AgentFlow again.
...
agentflow --wipe-project-memory -m "Update the authentication system"
```
...
agentflow --wipe-project-memory -m "Implement payment processing system"
```
...
agentflow -m "Show me the project structure"
# Look for the 💾 Memory: X facts, Y snippets, Z notes line
```

### Example 4: Using memory during ongoing development

When working on a complex feature over multiple sessions, memory allows AgentFlow to:
