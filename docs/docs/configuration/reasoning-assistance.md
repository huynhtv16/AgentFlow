...
Reasoning Assistance is a feature in AgentFlow that helps weaker models make better decisions about tool usage and task planning. It leverages a stronger model (typically your expert model) to provide strategic guidance to the main agent model at the beginning of each agent stage.

### Reasoning Assistance vs. Expert Model

It's important to understand the distinction between two related features in AgentFlow:
...
When reasoning assistance is enabled, AgentFlow performs the following steps at the beginning of each agent stage (research, planning, implementation):
...
agentflow -m "Your task description" --reasoning-assistance

# Disable reasoning assistance (overrides model defaults)
agentflow -m "Your task description" --no-reasoning-assistance
```
...
agentflow --model qwen-32b-coder-instruct --expert-model qwen-qwq-32b --reasoning-assistance -m "Create a simple web server in Python" 
```
...
agentflow -m "Create a simple web server in Python" --model claude-3-opus-20240229 --no-reasoning-assistance
```
