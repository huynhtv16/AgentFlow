...
AgentFlow includes a powerful and flexible logging system that helps you troubleshoot issues and monitor the tool's operation. This document explains how to configure and use the logging features effectively.

## Overview

The logging system in AgentFlow provides:
...
AgentFlow provides the following command line options to control logging behavior:
...
```bash
agentflow -m "Add new feature" --log-mode file
```
...
```bash
agentflow -m "Add new feature" --log-level debug
```
...
```bash
agentflow -m "Add new feature" --pretty-logger
```
...
When `--log-mode=file` is used, AgentFlow creates and maintains log files with the following characteristics:

### Location

Log files are stored in the `.agentflow/logs/` directory in your current working directory:

```
.agentflow/logs/agentflow_YYYYMMDD_HHMMSS.log
```

AgentFlow automatically creates this directory if it doesn't exist.

### Project State Directory

By default, logs are stored in the `.agentflow` directory in your current working directory. However, you can customize this location using the `--project-state-dir` flag:
...
Example: `agentflow_20250301_143027.log`

### Log Rotation

AgentFlow uses automatic log rotation to manage log file size and prevent excessive disk usage:
...
```bash
agentflow -m "Add new feature" --log-mode console --log-level debug
```
...
```bash
agentflow -m "Add new feature" --log-mode console --log-level info
```
...
- Examine log files in `.agentflow/logs/` for historical issues
...
Example:
```
2025-03-01 14:30:27,123 - agentflow.agent_utils - WARNING - Command execution timeout after 60 seconds