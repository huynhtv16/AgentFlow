...
Welcome to the AgentFlow community! We're thrilled you're interested in contributing. This project thrives thanks to contributors like you, and we're excited to have you join us on this journey.

## Ways to Contribute

There are many valuable ways to contribute to AgentFlow:
...
### 2. Report Issues
- Found a bug? Open an issue on our [GitHub repository](<repository-url>)
...
- You can use AgentFlow itself to help draft documentation changes
...
- **You can even use AgentFlow to help understand the codebase and make changes**
...
```bash
agentflow -m "Your task or query here"
```

5. Frontend miscellany:

The backend must always be run using `agentflow --server` separately. `yarn dev` is used to run the frontend development server.
...
## Migrations
See `agentflow/database/MIGRATIONS.md` for reference on how to run migrations.

## This is Your Project Too

AgentFlow is a community project that grows stronger with each contribution. Whether it's fixing a typo in documentation, reporting a bug, or adding a new feature - every contribution matters and is valued.
