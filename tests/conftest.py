...
def mock_trajectory_repository():
    ...
    with patch(
        "agentflow.database.repositories.trajectory_repository.TrajectoryRepository"
    ) as mock:
...
def mock_human_input_repository():
    ...
    with patch(
        "agentflow.database.repositories.human_input_repository.HumanInputRepository"
    ) as mock:
        # Setup a mock repository
        mock_repo = MagicMock()
...
def mock_session_repository():
    ...
    with patch(
        "agentflow.database.repositories.session_repository.SessionRepository"
    ) as mock:
        # Setup a mock repository
        mock_repo = MagicMock()
        ...
        # Set the contextvar
        from agentflow.database.repositories.session_repository import session_repo_var
...
def mock_repository_access(
    ...
    with patch(
        "agentflow.database.repositories.trajectory_repository.get_trajectory_repository",
        return_value=mock_trajectory_repository,
    ):
        with patch(
            "agentflow.database.repositories.human_input_repository.get_human_input_repository",
            return_value=mock_human_input_repository,
        ):
            with patch(
                "agentflow.database.repositories.session_repository.get_session_repository",
...
def isolated_db_environment(tmp_path, monkeypatch):
    ...
    actual .agentflow directory in the current working directory.
    ...
    # Create the .agentflow directory in the temporary path
