...
from agentflow.file_listing import get_file_listing
...
def test_non_git_file_listing():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        agentflow_dir = Path(temp_dir) / ".agentflow"
        venv_dir = Path(temp_dir) / ".venv"
        os.makedirs(agentflow_dir)
        os.makedirs(venv_dir)
        
        # Create files in excluded directories
        agentflow_file = agentflow_dir / "config.json"
        venv_file = venv_dir / "activate"
        
        # Write content to all files
        for file_path in [file1, file2, file3, file4, agentflow_file, venv_file]:
...
def test_non_git_directory_with_excluded_dirs():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        excluded_dirs = [".agentflow", ".venv", ".git", ".aider", "__pycache__"]
