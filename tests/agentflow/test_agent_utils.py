...
from agentflow.agent_context import (
    agent_context,
)
from agentflow.agent_utils import (
    create_agent
)
from agentflow.models_params import DEFAULT_TOKEN_LIMIT, models_params
from agentflow.models_params import AgentBackendType
...
def mock_config_repository():
    ...
    with patch(
        "agentflow.database.repositories.config_repository.config_repo_var"
    ) as mock_repo_var:
        mock_repo = MagicMock()
...
def mock_trajectory_repository():
    ...
    with patch(
        "agentflow.database.repositories.trajectory_repository.trajectory_repo_var"
    ) as mock_repo_var:
...
def mock_human_input_repository():
    ...
    with patch(
        "agentflow.database.repositories.human_input_repository.human_input_repo_var"
    ) as mock_repo_var:
        # Setup a mock repository
        mock_repo = MagicMock()
...
def test_create_agent_anthropic(mock_model, mock_config_repository):
    ...
    with (
        patch("agentflow.agent_utils.create_react_agent") as mock_react,
        patch.dict("agentflow.models_params.models_params", {"anthropic": mock_anthropic_model_config})
    ):
...
def test_create_agent_openai(mock_model, mock_config_repository):
    ...
    with patch("agentflow.agent_utils.should_use_react_agent", return_value=False):
        with patch("agentflow.agent_utils.CiaynAgent") as mock_ciayn:
...
def test_create_agent_anthropic_token_limiting_enabled(
    ...
    with (
        patch("agentflow.agent_utils.create_react_agent") as mock_react,
        patch("agentflow.anthropic_token_limiter.get_model_token_limit") as mock_limit,
        patch.dict("agentflow.models_params.models_params", {"anthropic": mock_anthropic_model_config})
    ):
        mock_react.return_value = "react_agent"
...
def test_agent_context_depth():
    from agentflow.agent_context import agent_context, get_depth
...
def test_run_agent_stream(monkeypatch, mock_config_repository):
    from agentflow.agent_utils import _run_agent_stream
    ...
    monkeypatch.setattr(
        "agentflow.agent_utils.print_agent_output", fake_print_agent_output
    )
...
def test_execute_test_command_wrapper(monkeypatch):
    from agentflow.agent_utils import _execute_test_command_wrapper
    ...
    monkeypatch.setattr("agentflow.agent_utils.execute_test_command", fake_execute)
...
def test_handle_api_error_valueerror():
    ...
    from agentflow.agent_utils import _handle_api_error
...
def test_run_agent_with_retry_checks_crash_status(monkeypatch, mock_config_repository):
    ...
    from agentflow.agent_context import agent_context, mark_agent_crashed
    from agentflow.agent_utils import run_agent_with_retry
    ...
    # Apply mocks
    monkeypatch.setattr("agentflow.agent_utils._run_agent_stream", mock_run_agent_stream)
    monkeypatch.setattr(
        "agentflow.agent_utils._setup_interrupt_handling", mock_setup_interrupt_handling
    )
    monkeypatch.setattr(
        "agentflow.agent_utils._restore_interrupt_handling",
        mock_restore_interrupt_handling,
    )
    monkeypatch.setattr("agentflow.agent_utils.check_interrupt", lambda: None)

    # First, run without a crash - agent should be run
    with agent_context() as ctx:
        monkeypatch.setattr("agentflow.agent_context.is_crashed", mock_is_crashed)
        monkeypatch.setattr(
            "agentflow.agent_context.get_crash_message", mock_get_crash_message
        )
...
def test_run_agent_with_retry_handles_badrequest_error(
    ...
    from agentflow.agent_context import agent_context, is_crashed
    from agentflow.agent_utils import run_agent_with_retry
    from agentflow.exceptions import ToolExecutionError
    ...
    with agent_context() as ctx:
        monkeypatch.setattr(
            "agentflow.agent_context.mark_agent_crashed", mock_mark_agent_crashed
        )
        monkeypatch.setattr("agentflow.agent_context.is_crashed", mock_is_crashed)
...
def test_run_agent_with_retry_handles_api_badrequest_error(
    ...
    monkeypatch.setattr("agentflow.agent_utils.check_interrupt", lambda: None)
    monkeypatch.setattr("agentflow.agent_utils._handle_api_error", lambda *args: None)
    monkeypatch.setattr("agentflow.agent_utils.APIError", MockAPIError)
    monkeypatch.setattr(
        "agentflow.agent_context.mark_agent_crashed", mock_mark_agent_crashed
    )
    monkeypatch.setattr("agentflow.agent_context.is_crashed", mock_is_crashed)
...
@patch("agentflow.agent_utils.create_react_agent")
def test_agent_backend_selection(mock_create_react_agent, mock_config_repository, mock_model):
    ...
    with patch("agentflow.agent_utils.should_use_react_agent", return_value=True):
