...
def mock_openai():
    ...
    """
    with patch("agentflow.llm.ChatOpenAI") as mock:
...
def test_initialize_expert_openrouter(clean_env, mock_openai, monkeypatch):
    ...
    mock_openai.assert_called_once_with(
        ...
        timeout=180,
        max_retries=5,
        default_headers={"HTTP-Referer": "https://agentflow.ai", "X-Title": "AgentFlow"},
...
def test_get_available_openai_models_success():
    ...
    with mock.patch("agentflow.llm.OpenAI") as mock_client:
...
def test_select_expert_model_priority():
    ...
    with mock.patch(
        "agentflow.llm.get_available_openai_models", return_value=available_models
    ):
...
def test_provider_name_validation():
    ...
    for provider in providers:
        try:
            with patch("agentflow.llm.ChatOpenAI"), patch("agentflow.llm.ChatAnthropic"):
...
def mock_anthropic():
    ...
    Prevents actual API calls during testing.
    """
    with patch("agentflow.llm.ChatAnthropic") as mock:
...
def mock_gemini():
    ...
    with patch("agentflow.llm.ChatGoogleGenerativeAI") as mock:
...
def mock_deepseek_reasoner():
    ...
    with patch("agentflow.llm.ChatDeepseekReasoner") as mock:
...
def mock_deepseek():
    ...
    with patch("agentflow.llm.ChatDeepSeek") as mock:
        mock.return_value = Mock()
        yield mock
...
def test_makehub_price_performance_ratio(clean_env, mock_openai, monkeypatch):
    ...
    with patch("agentflow.llm.get_config_repository", return_value=mock_config_repo):
