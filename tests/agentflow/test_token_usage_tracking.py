...
def test_token_usage_storage():
    ...
    with patch('agentflow.agent_utils.get_config_repository') as mock_config:
        ...
        with patch('agentflow.callbacks.default_callback_handler.DefaultCallbackHandler', return_value=mock_cb):
                # Force completion to happen after one iteration
                with patch('agentflow.agent_utils.is_completed', return_value=True):
                    # Replace the trajectory repository with our mock
                    with patch('agentflow.agent_utils.get_trajectory_repository', return_value=mock_repo):
                        # Suppress check_interrupt and print_agent_output
                        with patch('agentflow.agent_utils.check_interrupt'), \
                             patch('agentflow.agent_utils.print_agent_output'), \
                             patch('agentflow.agent_utils.reset_completion_flags'):
                            ...
                            with patch('agentflow.agent_utils._run_agent_stream', side_effect=side_effect):
            ...
            with patch('agentflow.callbacks.default_callback_handler.DefaultCallbackHandler', return_value=mock_cb):
                    ...
                    with patch('agentflow.agent_utils.is_completed', return_value=True):
                        ...
                        with patch('agentflow.agent_utils.get_trajectory_repository', return_value=mock_repo):
                            ...
                            with patch('agentflow.agent_utils.logger.error') as mock_error_log:
