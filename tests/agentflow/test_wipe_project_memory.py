...
from agentflow.__main__ import wipe_project_memory, parse_arguments
...
def test_wipe_project_memory_function():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        # Create a fake .agentflow directory and pk.db file
        agentflow_dir = Path(os.path.join(temp_dir, ".agentflow"))
        os.makedirs(agentflow_dir, exist_ok=True)
...
def test_build_status_shows_reset_option():
    ...
    from agentflow.__main__ import build_status
    
    # Mock repositories to return different numbers of items
    with patch("agentflow.__main__.get_key_fact_repository") as mock_fact_repo, \
         patch("agentflow.__main__.get_key_snippet_repository") as mock_snippet_repo, \
         patch("agentflow.__main__.get_research_note_repository") as mock_note_repo, \
         patch("agentflow.__main__.get_config_repository") as mock_config_repo:
...
def test_main_with_wipe_project_memory_flag():
    ...
    with (patch("agentflow.__main__.wipe_project_memory", mock_wipe),
          patch("agentflow.__main__.parse_arguments", return_value=mock_args),
          patch("agentflow.__main__.setup_logging"),  # Mock setup_logging itself
          patch("agentflow.__main__.get_config_repository"),  # Mock other potential calls before exit
          patch("agentflow.__main__.launch_server"),
          patch("agentflow.__main__.DatabaseManager"),
          patch("agentflow.config.save_default_values")):  # Mock save_default_values
