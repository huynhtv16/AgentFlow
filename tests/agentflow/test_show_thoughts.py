...
def test_show_thoughts_config(mock_config_repository):
    ...
    with patch.object(sys, "argv", ["agentflow", "--show-thoughts"]):
        with patch("agentflow.__main__.parse_arguments", side_effect=mock_parse_arguments):
            # Mock ConfigRepositoryManager to return our mock
            with patch('agentflow.database.repositories.config_repository.ConfigRepositoryManager.__enter__',
                       return_value=mock_config_repository):
                # Mock the required dependencies to prevent actual execution
                with patch("agentflow.__main__.setup_logging"), \
                     patch("agentflow.__main__.DatabaseManager"), \
                     patch("agentflow.__main__.ensure_migrations_applied"), \
                     patch("agentflow.__main__.check_dependencies"), \
                     patch("agentflow.__main__.validate_environment", return_value=(True, [], True, [])), \
                     patch("agentflow.__main__.build_status"), \
                     patch("agentflow.__main__.console.print"), \
                     patch("agentflow.__main__.initialize_llm"), \
                     patch("agentflow.database.repositories.session_repository.get_session_repository", return_value=MagicMock(create_session=MagicMock())), \
                     patch("agentflow.__main__.run_research_agent"), \
                     patch("agentflow.__main__.main", return_value=None):  # Prevent actual main execution
    ...
    with patch.object(sys, "argv", ["agentflow"]):
        with patch("agentflow.__main__.parse_arguments", side_effect=mock_parse_arguments):
            # Mock ConfigRepositoryManager to return our mock
            with patch('agentflow.database.repositories.config_repository.ConfigRepositoryManager.__enter__',
                       return_value=mock_config_repository):
