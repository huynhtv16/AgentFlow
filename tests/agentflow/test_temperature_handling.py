...
from agentflow.llm import get_model_default_temperature, create_llm_client
...
class TestTemperatureHandling(unittest.TestCase):
    ...
    def test_get_model_default_temperature(self):
        ...
        with patch("agentflow.models_params.models_params", mock_models_params), \
             patch("agentflow.models_params.DEFAULT_TEMPERATURE", 0.7):
    ...
    def test_create_llm_client_temperature_handling(self):
        ...
        # Setup mocks
        with patch("agentflow.llm.models_params", mock_models_params), \
             patch("agentflow.models_params.models_params", mock_models_params), \
             patch("agentflow.models_params.DEFAULT_TEMPERATURE", 0.7), \
             patch("agentflow.llm.cpm") as mock_cpm, \
             patch("agentflow.llm.get_provider_config") as mock_get_provider_config, \
             patch("agentflow.llm.ChatAnthropic") as mock_chat_anthropic, \
             patch("agentflow.llm.LLM_REQUEST_TIMEOUT", 180), \
             patch("agentflow.llm.LLM_MAX_RETRIES", 5):
