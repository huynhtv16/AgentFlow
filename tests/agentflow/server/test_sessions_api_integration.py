...
This module contains integration tests for the API endpoints defined in agentflow/server/api_v1_sessions.py.
...
from agentflow.server.server import app
...
def mock_sessions():
    return [
        SessionModel(
            ...
            command_line=f"agentflow test{i + 1}",
            program_version="1.0.0",
...
def mock_repo(mock_session, mock_sessions):
    ...
    def create_with_metadata(metadata=None):
        ...
        return SessionModel(
            ...
            updated_at=now,
            start_time=now,
            command_line="agentflow test-custom" if metadata else "agentflow test-null",
            program_version="1.0.0",
