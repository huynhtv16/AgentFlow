...
This module contains tests for the sessions API endpoints in agentflow/server/api_v1_sessions.py.
...
from agentflow.server.api_v1_sessions import router, get_repository
from agentflow.database.pydantic_models import SessionModel, TrajectoryModel
...
def test_delete_session_success(client, mock_repo, mock_session):
    ...
    with patch("agentflow.server.api_v1_sessions.is_agent_running", return_value=True), \
            patch("agentflow.server.api_v1_sessions.stop_agent", return_value=True):
