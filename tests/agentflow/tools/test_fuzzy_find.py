...
def test_excluding_system_dirs(non_git_repo):
    ...
    (non_git_repo / ".agentflow").mkdir(exist_ok=True)
    (non_git_repo / ".agentflow/config.json").write_text('{"setting": "value"}')
    ...
    # Should find config.py but not .agentflow/config.json
    assert any("config.py" in match[0] for match in results)
    assert not any(".agentflow/config.json" in match[0] for match in results)
