...
from agentflow.database.connection import (
    DatabaseManager,
    close_db,
    db_var,
    get_db,
...
class TestInitDb:
    ...
    def test_init_db_creates_directory(self, cleanup_db, db_path_mock):
        """Test that init_db creates the .agentflow directory if it doesn't exist."""
        # Remove the .agentflow directory to test creation
        agentflow_dir = db_path_mock / ".agentflow"
        if agentflow_dir.exists():
            for item in agentflow_dir.iterdir():
                if item.is_file():
                    item.unlink()
            agentflow_dir.rmdir()
        ...
        assert agentflow_dir.exists()
        assert agentflow_dir.is_dir()
        assert isinstance(db, peewee.SqliteDatabase)
        assert not db.is_closed()
        assert hasattr(db, "_is_in_memory")
    ...
    def test_init_db_creates_database_file(self, cleanup_db, db_path_mock):
        ...
        assert (db_path_mock / ".agentflow" / "pk.db").exists()
        assert (db_path_mock / ".agentflow" / "pk.db").is_file()
    ...
    def test_in_memory_mode_no_directory_created(self, cleanup_db, db_path_mock):
        ...
        assert not (db_path_mock / ".agentflow" / "pk.db").exists()
...
class TestGetDb:
    ...
    def test_get_db_initializes_connection(self, cleanup_db):
        ...
        with patch("agentflow.database.connection.init_db") as mock_init_db:
...
class TestCloseDb:
    ...
    @patch("agentflow.database.connection.peewee.SqliteDatabase.close")
