...
from agentflow.project_state import (
...
@pytest.fixture
def agentflow_only_dir(tmp_path):
    """Create a directory with only a .agentflow directory."""
...
def mixed_allowed_dir(tmp_path):
    """Create a directory with all allowed items (.git, .gitignore, and .agentflow)."""
    git_dir = tmp_path / ".git"
    git_dir.mkdir(exist_ok=True)
    gitignore = tmp_path / ".gitignore"
    gitignore.write_text("*.pyc\n")
...
def test_agentflow_only_directory(agentflow_only_dir):
    """Test that a directory with only a .agentflow directory is considered a new project."""
    assert is_new_project(str(agentflow_only_dir)) is True
...
def test_verify_fix(tmp_path):
    """
    Verify fix: a project with only .agentflow directory is considered new,
    but adding other files makes it recognized as an existing project.
    """
    # Create a .agentflow directory inside the temporary directory
    agentflow_dir = tmp_path / ".agentflow"
    agentflow_dir.mkdir(exist_ok=True)

    # Check that is_new_project() returns True (only .agentflow directory)
