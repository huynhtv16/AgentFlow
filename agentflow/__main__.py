...
from agentflow import print_error, print_stage_header
from agentflow.__version__ import __version__
from agentflow.version_check import check_for_newer_version
from agentflow.agent_utils import (
    create_agent,
    run_agent_with_retry,
)
from agentflow.agents.research_agent import run_research_agent
...
from agentflow.database.repositories.key_fact_repository import (
    KeyFactRepositoryManager,
    get_key_fact_repository,
)
from agentflow.database.repositories.key_snippet_repository import (
    KeySnippetRepositoryManager,
    get_key_snippet_repository,
)
from agentflow.database.repositories.human_input_repository import (
    HumanInputRepositoryManager,
    get_human_input_repository,
)
from agentflow.database.repositories.research_note_repository import (
    ResearchNoteRepositoryManager,
    get_research_note_repository,
)
from agentflow.database.repositories.trajectory_repository import (
    TrajectoryRepositoryManager,
    get_trajectory_repository,
)
from agentflow.database.repositories.session_repository import SessionRepositoryManager
from agentflow.database.repositories.related_files_repository import (
    RelatedFilesRepositoryManager,
)


from agentflow.database.repositories.work_log_repository import WorkLogRepositoryManager
from agentflow.database.repositories.config_repository import (
    ConfigRepositoryManager,
    get_config_repository,
)
from agentflow.env_inv import EnvDiscovery
from agentflow.env_inv_context import EnvInvManager, get_env_inv
from agentflow.model_formatters import format_key_facts_dict
from agentflow.model_formatters.key_snippets_formatter import format_key_snippets_dict
from agentflow.console.formatting import cpm
from agentflow.database import (
    DatabaseManager,
    ensure_migrations_applied,
)
from agentflow.dependencies import check_dependencies
from agentflow.env import validate_environment
from agentflow.exceptions import AgentInterrupt
from agentflow.fallback_handler import FallbackHandler
from agentflow.llm import initialize_llm, get_model_default_temperature
from agentflow.logging_config import get_logger, setup_logging
from agentflow.models_params import models_params
from agentflow.project_info import format_project_info, get_project_info
from agentflow.prompts.chat_prompts import CHAT_PROMPT
from agentflow.prompts.web_research_prompts import WEB_RESEARCH_PROMPT_SECTION_CHAT
from agentflow.prompts.custom_tools_prompts import DEFAULT_CUSTOM_TOOLS_PROMPT
from agentflow.server.server import app as fastapi_app
from agentflow.tool_configs import get_chat_tools, set_modification_tools, get_custom_tools
from agentflow.tools.human import ask_human
...
def launch_server(host: str, port: int, args):
    """Launch the AgentFlow web interface."""
    from agentflow.database.connection import DatabaseManager
    from agentflow.database.repositories.session_repository import SessionRepositoryManager
    from agentflow.database.repositories.key_fact_repository import (
        KeyFactRepositoryManager,
    )
    ...
    from agentflow.database.repositories.work_log_repository import (
        WorkLogRepositoryManager,
    )
    from agentflow.database.repositories.config_repository import ConfigRepositoryManager
    from agentflow.env_inv_context import EnvInvManager
    from agentflow.env_inv import EnvDiscovery
    ...
    from agentflow.database import ensure_migrations_applied
    ...
    print(f"Starting AgentFlow web interface on http://{host}:{port}")
...
def parse_arguments(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="AgentFlow - AI Agent for executing programming and research tasks.",
    ...
    parser.add_argument(
        "--wipe-project-memory",
        action="store_true",
        help="Delete the project database file (.agentflow/pk.db) before starting, effectively wiping all stored memory",
    )
    ...
    parser.epilog = """
Examples:
  agentflow -m "Add error handling to the database module"
  agentflow --server
  agentflow last-cost
  ...
  agentflow extract-changelog 0.25.0
    ...
    # If only 'agentflow' is run with no subcommand or message, it should show help or default to agent.
    ...
    # This needs careful handling if we want `agentflow "my task"` to work without -m.
    # Current setup: `agentflow -m "task"` or `agentflow <subcommand>`.
...
def handle_last_cost(args):
    import json
    from agentflow.scripts.last_session_usage import get_latest_session_usage
...
def handle_all_costs(args):
    import json
    from agentflow.scripts.all_sessions_usage import get_all_sessions_usage
...
def handle_extract_plan(args):
    from agentflow.scripts.extract_plan import get_plan_for_session
    from agentflow.scripts.extract_last_plan import main as extract_last_plan_main
...
def handle_extract_last_research_notes(args):
    from agentflow.scripts.extract_last_research_notes import main as extract_last_research_notes_main
    extract_last_research_notes_main(project_state_dir=args.project_state_dir)
    sys.exit(0)

def handle_generate_openapi(args):
    from agentflow.scripts.generate_openapi import main as generate_openapi_main
    generate_openapi_main() # This script prints to stdout
    sys.exit(0)

def handle_create_migration(args):
    from agentflow.database.migrations import create_new_migration
    ...
    # If `project_state_dir` is set, `create_new_migration` needs to know where `.agentflow/migrations` should be.
...
def handle_migrate(args):
    from agentflow.database.migrations import ensure_migrations_applied
...
def handle_migration_status(args):
    from agentflow.database.migrations import get_migration_status
...
def wipe_project_memory(custom_dir=None):
    ...
    Args:
        custom_dir: Optional custom directory to use instead of .agentflow in current directory
    ...
    if custom_dir:
        agentflow_dir = Path(custom_dir)
        db_path = os.path.join(custom_dir, "pk.db")
    else:
        cwd = os.getcwd()
        agentflow_dir = Path(os.path.join(cwd, ".agentflow"))
        db_path = os.path.join(agentflow_dir, "pk.db")
...
def main():
    """Main entry point for the agentflow command line tool."""
    ...
    logger.debug("Starting AgentFlow with arguments: %s", args)
    ...
    from agentflow.config import save_default_values
        ...
        with DatabaseManager(base_dir=args.project_state_dir) as db:
            ...
            with (
                ...
                console.print(
                    Panel(
                        status,
                        title=f"AgentFlow v{__version__}",
                ...
                if args.research_and_plan_only:
                    ...
                    print_stage_header(
                        ...
                        "You can extract the plan using 'agentflow extract-last-plan' "
                        "and research notes using 'agentflow extract-last-research-notes'."
                    )
                    sys.exit(0)
