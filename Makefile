...
test:
	...
	@echo "--> Running tests..."
	$(VENV_PYTHON) -m pytest --cov=agentflow --cov-report=term-missing --cov-report=html
...
migrate-create:
	...
	$(VENV_PYTHON) -m agentflow create-migration $(name)

migrate:
	@echo "--> Running database migrations..."
	$(VENV_PYTHON) -m agentflow migrate

migrate-status:
	@echo "--> Checking migration status..."
	$(VENV_PYTHON) -m agentflow migration-status
...
check-model:
	...
	@$(VENV_PYTHON) -m agentflow.scripts.check_model $(model)

extract-last-plan:
	@$(VENV_PYTHON) -m agentflow extract-last-plan

extract-last-research-notes:
	@$(VENV_PYTHON) -m agentflow extract-last-research-notes

last-cost:
	$(VENV_PYTHON) -m agentflow last-cost

all-costs:
	$(VENV_PYTHON) -m agentflow all-costs

extract-plan:
	@if [ -z "$(session_id)" ]; then \
		$(VENV_PYTHON) -m agentflow extract-plan; \
	else \
		$(VENV_PYTHON) -m agentflow extract-plan $(session_id); \
	fi
