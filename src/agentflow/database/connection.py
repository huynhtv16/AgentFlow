"""
Database connection management for agentflow.
...
class DatabaseManager:
    ...
    def __enter__(self) -> peewee.SqliteDatabase:
        ...
        try:
            ...
            from agentflow.database.models import initialize_database
            initialize_database()
...
def init_db(in_memory: bool = False, base_dir: Optional[str] = None) -> peewee.SqliteDatabase:
    ...
    Creates the .agentflow directory if it doesn't exist and initializes
    ...
    if in_memory:
        ...
        if base_dir:
            ...
            agentflow_dir = Path(base_dir)
            agentflow_dir = agentflow_dir.absolute()  # Ensure we have the absolute path
            agentflow_dir_str = str(agentflow_dir)
        else:
            # Use .agentflow in the current working directory
            agentflow_dir_str = os.path.join(cwd, ".agentflow")
            agentflow_dir = Path(agentflow_dir_str)
            agentflow_dir = agentflow_dir.absolute()  # Ensure we have the absolute path
            agentflow_dir_str = str(
                agentflow_dir
            )  # Update string representation with absolute path

        logger.debug(f"Creating database directory at: {agentflow_dir_str}")
        ...
        if not os.path.exists(agentflow_dir_str):
            try:
                logger.debug("Attempting directory creation with os.mkdir")
                os.mkdir(agentflow_dir_str, mode=0o755)
                directory_created = os.path.exists(agentflow_dir_str) and os.path.isdir(
                    agentflow_dir_str
                )
        ...
        if not directory_created:
            try:
                ...
                os.makedirs(agentflow_dir_str, exist_ok=True, mode=0o755)
                directory_created = os.path.exists(agentflow_dir_str) and os.path.isdir(
                    agentflow_dir_str
                )
                if directory_created:
        ...
        if not directory_created:
            try:
                ...
                agentflow_dir.mkdir(mode=0o755, parents=True, exist_ok=True)
                directory_created = os.path.exists(agentflow_dir_str) and os.path.isdir(
                    agentflow_dir_str
                )
                if directory_created:
        ...
        path_exists = agentflow_dir.exists()
        os_exists = os.path.exists(agentflow_dir_str)
        is_dir = os.path.isdir(agentflow_dir_str) if os_exists else False
        ...
        try:
            parent_dir = os.path.dirname(agentflow_dir_str)
        ...
        if not os_exists or not is_dir:
            error_msg = f"Directory does not exist or is not a directory after creation attempts: {agentflow_dir_str}"
            logger.error(error_msg)
            if error_messages:
                logger.error(f"Previous errors: {', '.join(error_messages)}")
            raise FileNotFoundError(f"Failed to create directory: {agentflow_dir_str}")

        # Check directory permissions
        try:
            permissions = oct(os.stat(agentflow_dir_str).st_mode)[-3:]
            logger.debug(
                f"Directory created/verified: {agentflow_dir_str} with permissions {permissions}"
            )

            # List directory contents for debugging
            dir_contents = os.listdir(agentflow_dir_str)
        ...
        db_path = os.path.join(agentflow_dir_str, "pk.db")
