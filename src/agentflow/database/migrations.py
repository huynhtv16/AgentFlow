"""
Database migrations for agentflow.
...
from agentflow.database.connection import DatabaseManager, get_db
...
class MigrationManager:
    """
    Manages database migrations for the agentflow application.
    ...
    def __init__(
        ...
        if db_path is None:
            ...
            cwd = os.getcwd()
            agentflow_dir = os.path.join(cwd, ".agentflow")
    ...
    def _get_source_package_migrations_dir(self) -> str:
        ...
        """
        try:
            # Get the base directory of the agentflow package
...
def ensure_migrations_applied() -> bool:
    ...
    Creates the .agentflow directory if it doesn't exist,
    ...
    """
    try:
        # Ensure .agentflow directory exists for the database file
        cwd = os.getcwd()
        agentflow_dir = os.path.join(cwd, ".agentflow")
        ...
        import agentflow
        package_dir = os.path.dirname(os.path.abspath(agentflow.__file__))
        ...
        err_msg = f"Failed to ensure .agentflow directory exists or initialize migrations: {str(e_init)}"
        logger.exception(err_msg)
        return False, err_msg
