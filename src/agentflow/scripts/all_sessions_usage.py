...
def get_all_sessions_usage(project_dir: Optional[str] = None, db_path: Optional[str] = None) -> Tuple[List[Dict[str, Any]], int]:
    ...
    Args:
        project_dir: Optional directory path where the .agentflow folder is located.
    ...
    try:
        ...
        if db_path:
            ...
            # Check if .agentflow directory exists in the specified project directory
            agentflow_dir = Path(os.path.join(base_dir, ".agentflow"))
            if not agentflow_dir.exists():
                return [create_empty_result(f"No .agentflow directory found in {project_dir}")], 1
...
def main():
    ...
    parser.add_argument("--project-dir", help="Directory containing the .agentflow folder (defaults to current directory)")
